// DownloadingNewsThread.cpp : this file initializes the news relvent objects and also it load the data to the objects
...
void DownloadingNewsThread::operator()(NewsCommonObjects& common) //downloading articles from News API site
{
	...
	if (res->status) {
		...
		for (auto& articleContent : json_result["articles"]) { //initialize article related values with its suitable values
			Article article;
		...
		common.isDataReady = true;
		common.isDataReady.notify_one();//notifying initNewsWindow thread to run because the data have been downloaded
	}
}